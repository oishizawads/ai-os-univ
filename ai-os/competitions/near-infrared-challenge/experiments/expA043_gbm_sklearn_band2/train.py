...
SETTINGS = [
    ("snv_pca20_gbm",     apply_snv,     20, {"n_estimators":500,"learning_rate":0.05,"max_depth":4,"min_samples_leaf":10,"subsample":0.8,"random_state":42}),
    ("snv_sg1_pca20_gbm", apply_snv_sg1, 20, {"n_estimators":500,"learning_rate":0.05,"max_depth":4,"min_samples_leaf":10,"subsample":0.8,"random_state":42}),
    ("snv_pca10_gbm",     apply_snv,     10, {"n_estimators":500,"learning_rate":0.05,"max_depth":4,"min_samples_leaf":10,"subsample":0.8,"random_state":42}),
]
...
def main():
    ...
    for name, pfn, pca_n, params in SETTINGS:
        ...
        for fold, (tr, val) in enumerate(logo.split(Xp, y, groups), 1):
            ...
            m = GradientBoostingRegressor(**params); m.fit(Xtr, y[tr]); oof[val] = m.predict(Xval)
    ...
    m = GradientBoostingRegressor(**params); m.fit(Xtr, y); preds = m.predict(Xtest)
    ...
    save_submission(sample_submit_df, test_df, sample_col, preds, SUBMISSIONS_DIR/f"expA043_gbm_{name}_submission.csv")
    print(f"saved_submission=expA043_gbm_{name}_submission.csv")
