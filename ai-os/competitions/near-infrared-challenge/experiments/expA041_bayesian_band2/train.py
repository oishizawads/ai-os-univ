...
SETTINGS = [
    ("snv_pca20_bayes",     apply_snv,     20, "bayes"),
    ("snv_pca30_bayes",     apply_snv,     30, "bayes"),
    ("snv_sg1_pca20_bayes", apply_snv_sg1, 20, "bayes"),
    ("snv_pca20_ard",       apply_snv,     20, "ard"),
    ("snv_pca10_ard",       apply_snv,     10, "ard"),
]
...
def main():
    ...
    for name, pfn, pca_n, mtype in SETTINGS:
        ...
        for fold, (tr, val) in enumerate(logo.split(Xp, y, groups), 1):
            ...
            m = BayesianRidge(max_iter=300) if mtype == "bayes" else ARDRegression(max_iter=300)
        ...
        if rmse < best_rmse: best_rmse, best_cfg = rmse, (name, pfn, pca_n, mtype)
    ...
    save_submission(sample_submit_df, test_df, sample_col, preds, SUBMISSIONS_DIR/f"expA041_bayes_{name}_submission.csv")
    print(f"saved_submission=expA041_bayes_{name}_submission.csv")
