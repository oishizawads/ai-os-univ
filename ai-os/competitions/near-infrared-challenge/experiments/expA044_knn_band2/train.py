...
SETTINGS = [
    ("snv_knn5",        apply_snv,     None, {"n_neighbors":5,  "weights":"distance","metric":"cosine"}),
    ("snv_knn10",       apply_snv,     None, {"n_neighbors":10, "weights":"distance","metric":"cosine"}),
    ("snv_knn20",       apply_snv,     None, {"n_neighbors":20, "weights":"distance","metric":"cosine"}),
    ("snv_pca20_knn5",  apply_snv,     20,   {"n_neighbors":5,  "weights":"distance","metric":"euclidean"}),
    ("snv_pca20_knn10", apply_snv,     20,   {"n_neighbors":10, "weights":"distance","metric":"euclidean"}),
    ("snv_sg1_knn10",   apply_snv_sg1, None, {"n_neighbors":10, "weights":"distance","metric":"cosine"}),
]
...
def main():
    ...
    for name, pfn, pca_n, params in SETTINGS:
        ...
        for fold, (tr, val) in enumerate(logo.split(Xp, y, groups), 1):
            if pca_n:
                ...
                Xtr, Xval = Xp[tr], Xp[val]
            m = KNeighborsRegressor(**params); m.fit(Xtr, y[tr]); oof[val] = m.predict(Xval)
    ...
    if pca_n:
        ...
        Xtr, Xtest = Xp_all, Xp_test
    m = KNeighborsRegressor(**params); m.fit(Xtr, y); preds = m.predict(Xtest)
    ...
    save_submission(sample_submit_df, test_df, sample_col, preds, SUBMISSIONS_DIR/f"expA044_knn_{name}_submission.csv")
    print(f"saved_submission=expA044_knn_{name}_submission.csv")
