...
from experiments.expA005_deriv_corr_pls.settings import (
    ID_COL,
    MODEL_PATH,
    SAMPLE_SUBMIT_PATH,
    SUBMISSION_PATH,
...
def main() -> None:
    ...
    selected_idx = payload["selected_feature_indices"]

    x_test = test_df[feature_cols].to_numpy(dtype=float)
    x_test = apply_deriv(x_test)
    x_test = x_test[:, selected_idx]

    test_df[TARGET_COL] = model.predict(x_test).ravel()
    ...
    print(f"best_n_top_features={payload['best_n_top_features']}")
    print(f"best_n_components={payload['best_n_components']}")
    print(f"selected_features={len(selected_idx)}")
