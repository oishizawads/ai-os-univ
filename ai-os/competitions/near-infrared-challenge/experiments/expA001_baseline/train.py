...
from experiments.expA001_baseline.settings import (
    ALPHA,
    EXPERIMENT_ID,
    MODEL_PATH,
    MODEL_NAME,
...
def main() -> None:
    ...
    print(f"n_samples={len(train_df)} n_features={len(feature_cols)} n_groups={train_df[GROUP_COL].nunique()}")
    ...
    for fold, (train_idx, valid_idx) in enumerate(splitter.split(x, y, groups=groups), start=1):
        ...
        valid_groups = sorted(pd.unique(groups[valid_idx]).tolist())
        print(
            f"fold={fold} rmse={fold_rmse:.6f} "
            f"train_size={len(train_idx)} valid_size={len(valid_idx)} valid_groups={valid_groups}"
        )
