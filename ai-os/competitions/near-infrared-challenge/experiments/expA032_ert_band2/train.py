...
SETTINGS = [
    ("snv_ert",     apply_snv,     {"n_estimators": 500, "max_features": "sqrt", "min_samples_leaf": 2, "random_state": 42, "n_jobs": -1}),
    ("snv_sg1_ert", apply_snv_sg1, {"n_estimators": 500, "max_features": "sqrt", "min_samples_leaf": 2, "random_state": 42, "n_jobs": -1}),
]
...
def main():
    ...
    m = ExtraTreesRegressor(**best_params)
    m.fit(Xp_all, y)
    ...
    save_submission(sample_submit_df, test_df, sample_col, preds, SUBMISSIONS_DIR / f"expA032_ert_{best_name}_submission.csv")
    print(f"saved_submission=expA032_ert_{best_name}_submission.csv")
