...
SETTINGS = [
    ("band2_snv_xgb", None),
    ("band2_snv_pca10_xgb", 10),
    ("band2_snv_pca20_xgb", 20),
]
...
def build_features(X_train, X_valid, pca_components):
    ...
    if pca_components is None:
        cols = [f"b{i}" for i in range(X_train_snv.shape[1])]
        return pd.DataFrame(X_train_snv, columns=cols), pd.DataFrame(X_valid_snv, columns=cols)
    pca = PCA(n_components=pca_components, random_state=42)
    X_tr = pca.fit_transform(X_train_snv)
    X_val = pca.transform(X_valid_snv)
    cols = [f"pca_{i}" for i in range(pca_components)]
    return pd.DataFrame(X_tr, columns=cols), pd.DataFrame(X_val, columns=cols)
...
def main():
    ...
    submission_path = SUBMISSIONS_DIR / f"expA029_xgb_band2_{best_name}_submission.csv"
