"""
LightGBM + Optuna hyperparameter tuning on band2+SNV.
CV: LeaveOneGroupOut (species). Objective: LOSO RMSE (clip>=0).
"""
from __future__ import annotations
import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

import numpy as np
import pandas as pd
import optuna
from lightgbm import LGBMRegressor
from sklearn.metrics import mean_squared_error
from sklearn.model_selection import LeaveOneGroupOut

from _base_band2 import (
    load_data, select_band2, apply_snv, apply_snv_sg1,
    save_submission, SUBMISSIONS_DIR
)

if hasattr(sys.stdout, "reconfigure"):
    sys.stdout.reconfigure(encoding="utf-8")

optuna.logging.set_verbosity(optuna.logging.WARNING)

EXPERIMENT_DIR = Path(__file__).resolve().parent
RESULTS_PATH = EXPERIMENT_DIR / "results.csv"
BEST_PARAMS_PATH = EXPERIMENT_DIR / "best_params.json"

N_TRIALS = 100
FIXED_PARAMS = {
    "objective": "regression",
    "metric": "rmse",
    "verbose": -1,
    "n_jobs": -1,
    "random_state": 42,
}


def loso_rmse(params: dict, X: np.ndarray, y: np.ndarray, groups: np.ndarray) -> float:
    logo = LeaveOneGroupOut()
    oof = np.zeros_like(y, dtype=np.float64)
    for tr, val in logo.split(X, y, groups):
        m = LGBMRegressor(**params)
        m.fit(X[tr], y[tr])
        oof[val] = m.predict(X[val])
    return float(np.sqrt(mean_squared_error(y, np.clip(oof, 0, None))))


def make_objective(X: np.ndarray, y: np.ndarray, groups: np.ndarray):
    def objective(trial: optuna.Trial) -> float:
        params = {
            **FIXED_PARAMS,
            "n_estimators":      trial.suggest_int("n_estimators", 200, 1000, step=100),
            "learning_rate":     trial.suggest_float("learning_rate", 0.01, 0.2, log=True),
            "num_leaves":        trial.suggest_int("num_leaves", 15, 127),
            "max_depth":         trial.suggest_int("max_depth", 3, 10),
            "min_child_samples": trial.suggest_int("min_child_samples", 5, 50),
            "subsample":         trial.suggest_float("subsample", 0.5, 1.0),
            "subsample_freq":    1,
            "colsample_bytree":  trial.suggest_float("colsample_bytree", 0.4, 1.0),
            "reg_alpha":         trial.suggest_float("reg_alpha", 1e-4, 10.0, log=True),
            "reg_lambda":        trial.suggest_float("reg_lambda", 1e-4, 10.0, log=True),
            "min_split_gain":    trial.suggest_float("min_split_gain", 0.0, 1.0),
        }
        return loso_rmse(params, X, y, groups)
    return objective


def main():
    EXPERIMENT_DIR.mkdir(parents=True, exist_ok=True)
    train_df, test_df, sample_submit_df, wave_cols, sample_col, species_col, target_col = load_data()
    band2_cols = select_band2(wave_cols)
    X_raw = train_df[band2_cols].to_numpy(dtype=np.float64)
    y = train_df[target_col].to_numpy(dtype=np.float64)
    groups = train_df[species_col].to_numpy()
    X_test_raw = test_df[band2_cols].to_numpy(dtype=np.float64)
    print(f"band2_features={len(band2_cols)}")

    results = []

    for preproc_name, preproc_fn in [("snv", apply_snv), ("snv_sg1", apply_snv_sg1)]:
        X = preproc_fn(X_raw)
        X_test = preproc_fn(X_test_raw)
        print(f"\n[Optuna] preproc={preproc_name}  n_trials={N_TRIALS}")

        study = optuna.create_study(
            direction="minimize",
            sampler=optuna.samplers.TPESampler(seed=42),
        )
        study.optimize(make_objective(X, y, groups), n_trials=N_TRIALS, show_progress_bar=False)

        best_trial = study.best_trial
        best_params = {**FIXED_PARAMS, **best_trial.params}
        best_loso = best_trial.value

        print(f"  best_loso={best_loso:.6f}")
        print(f"  best_params={best_trial.params}")

        # Re-evaluate with best params to get OOF
        final_rmse = loso_rmse(best_params, X, y, groups)
        results.append({
            "preproc": preproc_name,
            "loso_rmse": final_rmse,
            **best_trial.params,
        })

        # Save test predictions
        m_full = LGBMRegressor(**best_params)
        m_full.fit(X, y)
        preds = m_full.predict(X_test)
        print(f"  pred_min={preds.min():.4f}  pred_max={preds.max():.4f}")
        preds = np.clip(preds, 0, None)

        sub_path = SUBMISSIONS_DIR / f"expA049_lgbm_optuna_{preproc_name}_submission.csv"
        save_submission(sample_submit_df, test_df, sample_col, preds, sub_path)
        print(f"  saved_submission={sub_path.name}")

        # Save study trials
        trials_df = study.trials_dataframe()
        trials_df.to_csv(EXPERIMENT_DIR / f"trials_{preproc_name}.csv", index=False, encoding="utf-8")

    results_df = pd.DataFrame(results).sort_values("loso_rmse").reset_index(drop=True)
    results_df.to_csv(RESULTS_PATH, index=False, encoding="utf-8")
    print("\n=== Final results ===")
    print(results_df[["preproc", "loso_rmse"]].to_string(index=False))
    print(f"saved_results={RESULTS_PATH}")


if __name__ == "__main__":
    main()
